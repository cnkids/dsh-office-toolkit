...
| 工具 | 作用 |
| --- | --- |
| `office_read` | 读取 `.docx .doc .rtf .odt .xlsx .xls .xlsb .ods .csv .tsv`（Excel 以 TSV 返回） |
...
| **0.3.13** | `.docx` 生成改为自研（`docx@9`），去掉带 `postinstall` 的依赖，安装不再被 pnpm 打断 |
| **0.3.12** | 安全收口：`npm audit` 归零、防解压炸弹、移除图片探测路径 |
