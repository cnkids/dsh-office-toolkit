...
// ---------------------------------------------------------------------------
/**
 * Ordered content.xml → HTML rewrites. Each entry is `[pattern, replacement]`
 * where the replacement is a literal or a replacer function.
 * @type {Array<[RegExp, string | ((match: string, ...groups: string[]) => string)]>}
 */
const ODT_TAG_MAPS = [
  [/<office:automatic-styles[\s\S]*?<\/office:automatic-styles>/g, ''],
  [/<office:font-face-decls[\s\S]*?<\/office:font-face-decls>/g, ''],
  [/<office:scripts[\s\S]*?<\/office:scripts>/g, ''],
  [/<text:note[\s\S]*?<\/text:note>/g, ''],
  [/<text:line-break\s*\/>/g, '<br/>'],
  [/<text:tab\s*\/>/g, '\t'],
  [/<text:s\b[^>]*text:c="(\d+)"[^>]*\/>/g, (_m, n) => ' '.repeat(Number(n) || 1)],
  [/<text:s\s*\/>/g, ' '],
  [/<text:h(?=[\s>])(?:[^>]*outline-level="(\d)")?[^>]*>/g, (_m, lv) => `<h${clampHeading(lv ?? 1)}>`],
  [/<\/text:h>/g, '</h>'],
  [/<text:p\b[^>]*>/g, '<p>'],
  [/<\/text:p>/g, '</p>'],
  [/<text:list(?=[\s>])[^>]*>/g, '<ul>'],
  [/<\/text:list>/g, '</ul>'],
  [/<text:list-item(?=[\s>])[^>]*>/g, '<li>'],
  [/<\/text:list-item>/g, '</li>'],
  [/<table:table(?=[\s>])[^>]*>/g, '<table>'],
  [/<\/table:table>/g, '</table>'],
  [/<table:table-row(?=[\s>])[^>]*>/g, '<tr>'],
  [/<\/table:table-row>/g, '</tr>'],
  [/<table:table-cell(?=[\s>])[^>]*>/g, '<td>'],
  [/<\/table:table-cell>/g, '</td>'],
  [/<table:covered-table-cell(?=[\s>])[^>]*\/>/g, '<td></td>'],
  [/<text:a\b[^>]*xlink:href="([^"]*)"[^>]*>/g, '<a href="$1">'],
  [/<\/text:a>/g, '</a>'],
  [/<text:span\b[^>]*>/g, ''],
  [/<\/text:span>/g, ''],
];

const KEEP_TAGS = /^<\/?(p|h[1-6]|ul|li|table|tr|td|br|a)\b/;
...
export function odtXmlToHtml(contentXml) {
  const source = String(contentXml);
  const bodyMatch = /<office:body[\s\S]*<\/office:body>/.exec(source);
  let body = bodyMatch ? bodyMatch[0] : source;
  for (const [re, repl] of ODT_TAG_MAPS) body = body.replaceAll(re, repl);
  body = body.replaceAll(/<h([1-6])>([\s\S]*?)<\/h>/g, '<h$1>$2</h$1>');
  return body.replaceAll(/<[^>]+>/g, (tag) => (KEEP_TAGS.test(tag) ? tag : ''));
}
