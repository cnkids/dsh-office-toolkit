...
export function officePartOf(zip) {
  ...
  if (byTarget) {
    ...
    if (entry && kind) return { kind, path: byTarget, entry };
  }
  for (const [name, kind] of KNOWN_MAIN_PARTS) {
    const entry = partOf(zip, name);
    if (entry) return { kind, path: name, entry };
  }
  for (const name of Object.keys(zip.files)) {
    const entry = zip.files[name];
    if (entry.dir) continue;
    const kind = mainKindByName(name);
    if (kind) return { kind, path: name, entry };
  }
  return null;
}
...
function officeTargetFromRels(zip) {
  ...
  const at = xml.indexOf('officeDocument');
  if (at === -1) return '';
  const chunk = xml.slice(at, at + 400);
  const targetAt = chunk.indexOf('Target=');
  if (targetAt === -1) return '';
  const quote = chunk[targetAt + 7];
  const end = chunk.indexOf(quote, targetAt + 8);
  if (end === -1) return '';
  const raw = chunk.slice(targetAt + 8, end).replaceAll('\\', '/');
  let start = 0;
  while (raw[start] === '.' || raw[start] === '/') start += 1;
  return normalizeEntryName(raw.slice(start));
}
