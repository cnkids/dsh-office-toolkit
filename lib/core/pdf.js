 ...
 */
/** 按平台给的可信目录:Windows 留空(那里没有标准位置的 poppler,一律用随包 pdfjs)。 */
const BIN_DIRS_BY_PLATFORM = {
  darwin: ['/opt/homebrew/bin', '/usr/local/bin', '/opt/local/bin', '/usr/bin', '/bin'],
  win32: [],
};
const TRUSTED_BIN_DIRS = BIN_DIRS_BY_PLATFORM[process.platform] ?? ['/usr/local/bin', '/usr/bin', '/bin', '/snap/bin'];
...
/**
 * pdfjs 在 Node 下会为「渲染」缺 canvas / DOMMatrix / Path2D 打警告,还会抱怨没给
 * standardFontDataUrl(我们给了,它只是照例念一句)—— 只抽文本时这些是噪音。
 * 这里按消息前缀过滤掉这几类,其它日志照常输出,不做全局静音。
 */
const PDFJS_NOISE = ['Cannot polyfill', 'Cannot load "@napi-rs/canvas"', 'Ensure that the `standardFontDataUrl`'];

async function withPdfjsQuiet(run) {
  const { log, warn } = console;
  const filtered = (original) => (...args) => {
    const first = String(args[0] ?? '');
    if (PDFJS_NOISE.some((needle) => first.includes(needle))) return;
...
function loadPdfjs() {
  ...
  pdfjsPromise = pdfjsPromise || import(pathToFileURL(join(VENDOR, 'pdf.min.mjs')).href);
  return pdfjsPromise;
}

/**
 * pdfjs 需要的资源地址:
 *   - worker 走 `await import(workerSrc)`,必须是 file:// URL —— Node 的 ESM 加载器
 *     只认 file/data/node 三种 scheme,Windows 的 `C:\…` 裸路径会直接抛
 *     「Only URLs with a scheme in: file, data, and node」,fake worker 起不来;
 *   - cmaps / standard_fonts 走 Node 侧的 `fs.readFile(路径)`,必须是**文件系统路径**
 *     (file:// URL 会被当成文件名 → ENOENT)。
 * 两者形式不同,这里集中给出,顺带便于测试断言。
 */
export function pdfjsAssets() {
  return {
    workerUrl: pathToFileURL(join(VENDOR, 'pdf.worker.min.mjs')).href,
...
async function viaPdfjs(buf) {
  return withPdfjsQuiet(() => extractViaPdfjs(buf));
}

async function extractViaPdfjs(buf) {
