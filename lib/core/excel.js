...
/** 公式单元格的显示:mode 为 formula/both 时给出公式本体(共享公式给引用地址)。 */
function formulaDisplay(value, mode) {
  if (mode === 'value') return null;
  if (typeof value.formula !== 'string' || !value.formula) {
    return value.sharedFormula ? `=共享公式(${value.sharedFormula})` : null;
  }
  const formula = '=' + value.formula;
  if (mode === 'formula') return formula;
  const shown = value.result === undefined || value.result === null ? '' : cellDisplay(value.result, 'value');
  return shown ? `${formula} → ${shown}` : formula;
}

/** ExcelJS cell value -> display string. Objects may be errors/links/formulas. */
function cellDisplay(value, mode = 'value') {
  if (value === null || value === undefined) return '';
  if (value instanceof Date) return dateDisplay(value);
  if (typeof value !== 'object') return primitiveDisplay(value);
  return formulaDisplay(value, mode) ?? objectDisplay(value);
}
...
function objectDisplay(value) {
  ...
  // 没有缓存计算值(如本插件刚写的文件)时给出公式本体,并补上等号
  if (value.formula !== undefined) return '=' + String(value.formula);
  if (value.sharedFormula) return `=共享公式(${value.sharedFormula})`;
  return String(value.text ?? value);
}

function rowToCells(row, colCount, mode = 'value') {
  const out = [];
  for (let c = 1; c <= colCount; c++) {
    const cell = row.getCell(c);
    out.push(cellDisplay(cell.value, mode));
  }
...
export async function readWorkbook(buf, opts = {}) {
  ...
  for (const s of sheets) {
    ...
    if (s.truncated) parts.push(`> 行数超过窗口(${maxRows} 行)，要汇总/筛选/分组请改用 office_query（全表计算，只返回结论）；只看某一段再用 range/maxRows`);
...
function readSheetWindow(ws, opts) {
  ...
  ws.eachRow({ includeEmpty: true }, (row, rn) => {
    ...
    const cells = rowToCells(row, colCount, opts.formulaMode);
...
// ---------------------------------------------------------------------------
// office_query 的数据源:整表原始值矩阵(不做窗口截断,保留数字类型)
// ---------------------------------------------------------------------------
/** Cell value -> raw primitive: numbers stay numbers, dates become YYYY-MM-DD. */
function cellRaw(value) {
  if (value === null || value === undefined) return '';
  if (value instanceof Date) return dateDisplay(value);
  if (typeof value === 'object') return objectRaw(value);
  if (typeof value === 'string') return value.trim();
  return typeof value === 'number' && !Number.isFinite(value) ? '' : value;
}

/** Formula / rich-text / error cells: the cached result is what a spreadsheet would compute. */
function objectRaw(value) {
  const inner = value.result !== undefined && value.result !== null ? value.result : null;
  if (inner !== null) return cellRaw(inner);
  return objectDisplay(value).trim();
}

function rowToRawCells(row, colCount) {
  const out = [];
  for (let c = 1; c <= colCount; c++) out.push(cellRaw(row.getCell(c).value));
  return out;
}
...
function readSheetMatrix(ws, maxRows, maxCols) {
  const colCount = Math.min(Math.max(ws.columnCount || 0, 1), maxCols);
  const matrix = [];
  let truncated = false;
  ws.eachRow({ includeEmpty: true }, (row) => {
    if (matrix.length >= maxRows) { truncated = true; return; }
    const cells = rowToRawCells(row, colCount);
    // 带样式的空行很常见(Excel 常把整行算进使用范围),统计时跳过才贴近真实数据
    if (!isBlankRow(cells)) matrix.push(cells);
  });
  return { name: ws.name, matrix, truncated, totalRows: ws.rowCount || matrix.length };
}

/** Whole-workbook read for office_query: every sheet as a raw matrix, no row window. */
export async function readMatrixSheets(buf, opts = {}) {
  const wb = await loadWorkbook(buf, '不是有效的 .xlsx 文件(ExcelJS 无法解析): ');
  ...
  const sheets = [];
  wb.eachSheet((ws) => { sheets.push(readSheetMatrix(ws, maxRows, maxCols)); });
  return { sheets, sheetNames: wb.worksheets.map((ws) => ws.name) };
}
...
export async function editWorkbook(buf, ops) {
  ...
  // exceljs 从文件读入时会把区域型数据验证拆成逐格条目,写回时又合并错(见 mergeDataValidations)
  for (const ws of wb.worksheets) mergeDataValidations(ws);
...
// 数据验证写回前的合并
//
// exceljs 的 DataValidationsXform 读入时把 `C2:C100` 展开成 99 个逐格条目,写出时
// optimiseDataValidations() 按「地址字符串」排序(C10 排在 C2 前)再贪心合并,于是合并出
// `C10:C100` + `C2:C100` 两条重叠规则 —— 每读写一次就多一条。这里在写盘前按「相同规则 +
// 逐格矩形」自己合并,读多少写多少次都只留一条。
// ---------------------------------------------------------------------------
const MAX_MERGE_CELLS = 20000;
...
function cellKeyOf(address) {
  const match = /^([A-Z]+)(\d+)$/.exec(String(address).toUpperCase());
  return match ? { col: colLettersToIndex(match[1]), row: Number(match[2]) } : null;
}

/** 逐格地址 → 互不重叠的矩形(向下再向右贪心扩张)。 */
function rectanglesOf(cells) {
  ...
  for (const start of ordered) {
    ...
    let height = 1;
    while (pending.has(`${start.col}:${start.row + height}`)) height += 1;
    const columnHas = (col) => Array.from({ length: height }, (_, i) => `${col}:${start.row + i}`).every((key) => pending.has(key));
    let width = 1;
    while (columnHas(start.col + width)) width += 1;
    for (let i = 0; i < height; i += 1) {
      for (let j = 0; j < width; j += 1) pending.delete(`${start.col + j}:${start.row + i}`);
    }
    const end = { col: start.col + width - 1, row: start.row + height - 1 };
    const first = `${indexToLetters(start.col)}${start.row}`;
    rects.push(width > 1 || height > 1 ? `${first}:${indexToLetters(end.col)}${end.row}` : first);
  }
  return rects;
}

/** 把逐格数据验证合并回区域条目(区域键原样保留)。 */
export function mergeDataValidations(ws) {
  const model = ws.dataValidations?.model;
  const entries = Object.entries(model || {}).filter(([, value]) => value);
  const singles = entries.filter(([address]) => !String(address).includes(':'));
  if (!singles.length || singles.length > MAX_MERGE_CELLS) return;
  const groups = new Map();
  for (const [address, validation] of singles) {
    const cell = cellKeyOf(address);
    if (!cell) continue;
    const key = JSON.stringify(validation);
    if (!groups.has(key)) groups.set(key, { validation, cells: new Map() });
    groups.get(key).cells.set(`${cell.col}:${cell.row}`, cell);
  }
  const merged = {};
  for (const [address, validation] of entries) {
    if (String(address).includes(':')) merged[address] = validation;
  }
  for (const { validation, cells } of groups.values()) {
    for (const rect of rectanglesOf(cells)) merged[rect] = validation;
  }
  ws.dataValidations.model = merged;
}
...
// 条件格式与数据验证
// ---------------------------------------------------------------------------
const CF_TYPES = new Set(['cellIs', 'expression', 'colorScale', 'dataBar', 'iconSet', 'top10', 'aboveAverage', 'containsText', 'timePeriod']);
const CF_OPERATORS = new Set(['equal', 'notEqual', 'greaterThan', 'lessThan', 'greaterThanOrEqual', 'lessThanOrEqual', 'between', 'notBetween']);
const CF_TEXT_OPERATORS = new Set(['containsText', 'notContains', 'beginsWith', 'endsWith', 'containsBlanks', 'notContainsBlanks', 'containsErrors', 'notContainsErrors']);
const TIME_PERIODS = new Set(['today', 'yesterday', 'tomorrow', 'last7Days', 'thisWeek', 'lastWeek', 'nextWeek', 'thisMonth', 'lastMonth', 'nextMonth']);
const DV_TYPES = new Set(['list', 'whole', 'decimal', 'date', 'textLength', 'custom']);
const DV_OPERATORS = new Set(['between', 'notBetween', 'equal', 'notEqual', 'greaterThan', 'lessThan', 'greaterThanOrEqual', 'lessThanOrEqual']);
const ICON_STOPS = { 3: [0, 33, 67], 4: [0, 25, 50, 75], 5: [0, 20, 40, 60, 80] };
const RANGE_RE = /^[A-Z]+\d+(?::[A-Z]+\d+)?$/;

function requireChoice(value, allowed, what, fallback) {
  if (value === undefined || value === null || value === '') {
    if (fallback !== undefined) return fallback;
    throw new OfficeError(`${what} 是必填项，可为 ${[...allowed].join(' / ')}`);
  }
  const picked = String(value);
  if (!allowed.has(picked)) throw new OfficeError(`${what} 可为 ${[...allowed].join(' / ')}，收到「${picked}」`);
  return picked;
}

function requireRuleText(value, what) {
  const text = value === undefined || value === null ? '' : String(value);
  if (!text) throw new OfficeError(`conditional_format 的 ${what} 是必填项`);
  return text;
}

/** 条件格式的差异样式(dxf):只写真正给出的字段,exceljs 会生成对应的 dxf。 */
function cfStyle(style) {
  ...
  // 差异格式的填充走 bgColor,这是 OOXML dxf 的约定(与单元格 fill 的 fgColor 相反)
  if (style.fill) out.fill = { type: 'pattern', pattern: 'solid', bgColor: { argb: 'FF' + ARGB(style.fill) } };
  if (style.numFmt) out.numFmt = String(style.numFmt);
  if (style.border) {
    const side = { style: 'thin', color: { argb: 'FF' + ARGB(style.borderColor || '000000') } };
    out.border = { top: side, bottom: side, left: side, right: side };
  }
  return Object.keys(out).length ? out : undefined;
}

/** cellIs 的阈值:value / value2(between 用)或 values 数组。 */
function cfThresholds(rule, operator) {
  const list = Array.isArray(rule.values) ? rule.values : [rule.value, rule.value2].filter((v) => v !== undefined && v !== null);
  if (operator === 'between' || operator === 'notBetween') {
    if (list.length !== 2) throw new OfficeError('conditional_format 的 between / notBetween 需要 value 与 value2 两个阈值');
    return list;
  }
  if (!list.length) throw new OfficeError('conditional_format 需要 value（cellIs / top10 等的阈值）');
  return [list[0]];
}

function scaleStops(count) {
  if (count <= 2) return [{ type: 'min' }, { type: 'max' }];
  return [{ type: 'min' }, ...Array.from({ length: count - 2 }, (_, i) => ({ type: 'percentile', value: Math.round(((i + 1) * 100) / (count - 1)) })), { type: 'max' }];
}

function scaleColors(rule) {
  const given = Array.isArray(rule.colors) ? rule.colors.map(ARGB) : [];
  if (given.length && given.length !== 2 && given.length !== 3) {
    throw new OfficeError('conditional_format 的 colorScale 需要 2 个或 3 个颜色（colors: ["F8696B","63BE7B"]）');
  }
  return given.length ? given : ['FFFFFF', '638EC6'];
}

function iconStops(iconSet) {
  const count = Number(String(iconSet).charAt(0)) || 3;
  return (ICON_STOPS[count] ?? ICON_STOPS[3]).map((value) => ({ type: 'percent', value }));
}

const CF_BUILDERS = {
  cellIs: (rule) => {
    const operator = requireChoice(rule.operator, CF_OPERATORS, 'conditional_format 的 operator', 'greaterThan');
    return { type: 'cellIs', operator, formulae: cfThresholds(rule, operator) };
  },
  expression: (rule) => ({ type: 'expression', formulae: [requireRuleText(rule.formula ?? rule.value, 'formula')] }),
  colorScale: (rule) => {
    const colors = scaleColors(rule);
    return { type: 'colorScale', cfvo: scaleStops(colors.length), color: colors.map((argb) => ({ argb: 'FF' + argb })) };
  },
  dataBar: (rule) => ({ type: 'dataBar', cfvo: [{ type: 'min' }, { type: 'max' }], color: { argb: 'FF' + ARGB(rule.color || '638EC6') } }),
  iconSet: (rule) => {
    const iconSet = String(rule.iconSet || '3TrafficLights1');
    return { type: 'iconSet', iconSet, showValue: rule.showValue !== false, cfvo: iconStops(iconSet) };
  },
  top10: (rule) => ({ type: 'top10', rank: Number(rule.rank ?? 10), percent: Boolean(rule.percent), bottom: Boolean(rule.bottom) }),
  aboveAverage: (rule) => ({ type: 'aboveAverage', aboveAverage: rule.aboveAverage !== false }),
  containsText: (rule) => ({
    type: 'containsText',
    operator: requireChoice(rule.operator, CF_TEXT_OPERATORS, 'conditional_format 的 operator', 'containsText'),
    text: requireRuleText(rule.text, 'text'),
  }),
  timePeriod: (rule) => ({ type: 'timePeriod', timePeriod: requireChoice(rule.timePeriod, TIME_PERIODS, 'conditional_format 的 timePeriod', 'today') }),
};

/** 一条条件格式规则(style 只对 cellIs/expression/top10/containsText 这类有意义)。 */
export function conditionalRuleOf(rule) {
  const type = requireChoice(rule.type, CF_TYPES, 'conditional_format 的 type');
  const style = cfStyle(rule.style);
  return { ...CF_BUILDERS[type](rule), ...(style ? { style } : {}) };
}

function opConditionalFormat(wb, op) {
  const ws = getWorksheet(wb, op.sheet ?? 1);
  const ref = String(op.range || op.ref || '');
  if (!RANGE_RE.test(ref)) throw new OfficeError(`conditional_format 的 range 形如 A1:A20（收到「${ref}」）`);
  const rules = (Array.isArray(op.rules) ? op.rules : [isPlainObject(op.rule) ? op.rule : op]).map(conditionalRuleOf);
  ws.addConditionalFormatting({ ref, rules });
}

/** 数据验证的一条规则:下拉候选、数值区间、文本长度等。 */
function validationOf(op) {
  const spec = isPlainObject(op.rule) ? op.rule : op;
  const type = requireChoice(spec.type, DV_TYPES, 'data_validation 的 type', 'list');
  const formulae = validationFormulae(spec, type);
  return {
    type,
    formulae,
    allowBlank: spec.allowBlank !== false,
    ...(type === 'list' || type === 'custom' ? {} : { operator: requireChoice(spec.operator, DV_OPERATORS, 'data_validation 的 operator', 'between') }),
    showErrorMessage: spec.showErrorMessage ?? spec.error !== undefined,
    showInputMessage: spec.showInputMessage ?? (spec.prompt !== undefined || spec.promptTitle !== undefined),
    ...(spec.errorTitle === undefined ? {} : { errorTitle: String(spec.errorTitle) }),
    ...(spec.error === undefined ? {} : { error: String(spec.error) }),
    ...(spec.errorStyle === undefined ? {} : { errorStyle: String(spec.errorStyle) }),
    ...(spec.promptTitle === undefined ? {} : { promptTitle: String(spec.promptTitle) }),
    ...(spec.prompt === undefined ? {} : { prompt: String(spec.prompt) }),
  };
}

/** list 支持 values(["甲","乙"]) 或 source(区域/公式)；其余类型用 value / values。 */
function validationFormulae(spec, type) {
  if (type === 'list') {
    if (Array.isArray(spec.values)) {
      if (!spec.values.length) throw new OfficeError('data_validation 的 values 不能是空数组');
      const joined = spec.values.map((v) => String(v).replaceAll('"', '')).join(',');
      return [`"${joined}"`];
    }
    if (spec.source !== undefined) return [String(spec.source).replace(/^=/, '')];
    if (Array.isArray(spec.formulae)) return spec.formulae;
    throw new OfficeError('data_validation 的 list 需要 values: ["甲","乙"] 或 source: "=Sheet2!$A$1:$A$5"');
  }
  const first = spec.value === undefined ? spec.formula : spec.value;
  const list = Array.isArray(spec.values) ? spec.values : [first, spec.value2].filter((v) => v !== undefined && v !== null);
  if (!list.length) throw new OfficeError('data_validation 需要 value（或 values / formula）');
  return list.map((v) => (typeof v === 'number' ? v : String(v)));
}

function opDataValidation(wb, op) {
  const ws = getWorksheet(wb, op.sheet ?? 1);
  const range = String(op.range || op.ref || '');
  if (!RANGE_RE.test(range)) throw new OfficeError(`data_validation 的 range 形如 C2:C100（收到「${range}」）`);
  ws.dataValidations.add(range, validationOf(op));
}
...
const OP_HANDLERS = {
  ...
  conditional_format: opConditionalFormat,
  data_validation: opDataValidation,
};
