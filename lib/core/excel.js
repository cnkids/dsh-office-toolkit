...
import { OfficeError, isWithinBytes, CAPS, isPlainObject, fmtBytes, readPath, isAsciiDigit, isAsciiLetter } from './util.js';
...
function getWorksheet(wb, sel) {
  if (typeof sel === 'number') {
    const ws = wb.worksheets[sel];
    if (ws) return ws;
    throw new OfficeError(`工作表序号 ${sel} 不存在（共 ${wb.worksheets.length} 个）`);
  }
...
function colIndexToLetters(n) {
  let s = '';
  let rest = n;
  while (rest > 0) { const r = (rest - 1) % 26; s = String.fromCodePoint(65 + r) + s; rest = Math.floor((rest - 1) / 26); }
  return s;
}
...
function opAddChart() {
  // handled by editWorkbook post-processing (OOXML injection)
  throw new OfficeError('add_chart 需经 editWorkbook 处理', 'INTERNAL');
}
