...
export async function readDocx(buf, { html = false, formatting = false } = {}) {
  ...
  const parsed = await parseWithFallback(zip, buffer);
...
async function parseWithFallback(zip, buffer) {
  ...
  if (!builtin) {
    throw new OfficeError(
      'docx 里没有可读的正文(word/document.xml 缺失或为空)。请用 Word 打开后另存为 .docx 再试',
      'BAD_DOCX'
    );
  }
...
function builtinText(zip) {
  ...
  try {
    const doc = extractDocxFormat(documentXml.asText());
    ...
    return {
      ...
      messages: ['mammoth 无法解析该文件,已用内置解析器读取(表格与样式可能丢失)'],
