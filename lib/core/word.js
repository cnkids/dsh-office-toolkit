...
/** Unzip a docx and read raw [Content_Types].xml etc. — reserved for future structured edits. */
export async function readDocxRaw(buf) {
  let zip;
  try {
    zip = new PizZip(buf);
  } catch {
    throw new OfficeError('不是有效的 docx(无法解压)', 'BAD_DOCX');
  }
  const parts = [];
  zip.forEach((path, file) => {
    if (file.dir) return;
    if (/^word\/(document\.xml|styles\.xml|media\/)/.test(path)) {
      parts.push({ path, bytes: file.asNodeBuffer().length });
    }
  });
  return parts;
}
