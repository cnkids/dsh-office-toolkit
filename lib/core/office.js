...
function defaultIo() {
  return {
    ...
    async writeBuf(p, b) { return writePath(p, b); },
    async remove(p) {
...
async function writeLegacyWord(io, path, ext, source, params) {
  ...
  try {
    ...
    await io.writeBuf(tmpHtml, Buffer.from(`<html><head><meta charset="utf-8"><title>${title}</title></head><body>${html}</body></html>`));
...
async function convertHtmlViaExternal(io, html, srcPath, srcExt, dstPath, dstExt) {
  ...
  await io.writeBuf(tmp, Buffer.from('<html><head><meta charset="utf-8"></head><body>' + html + '</body></html>'));
  try {
...
async function loadSheetjs() {
  if (!_sheetjsMod) _sheetjsMod = await import('xlsx');
  return _sheetjsMod;
}
