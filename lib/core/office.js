...
import { looksLikePdf, readPdfText } from './pdf.js';
...
const READABLE_EXTS = new Set(['docx', 'doc', 'odt', 'rtf', 'pdf', 'xlsx', 'xls', 'xlsb', 'ods', 'csv', 'tsv', 'txt']);
 ...
 */
/** 内容其实是 PDF(改过后缀也认)时的嗅探结果。 */
function pdfSniff(buffer, ext) {
  if (!looksLikePdf(buffer)) return null;
  return { ext: 'pdf', buffer, entries: [], note: ext === 'pdf' ? '' : '文件内容其实是 PDF(.pdf)' };
}
...
async function sniffFormat(io, path, ext, preloaded) {
  ...
  const asPdf = pdfSniff(buffer, ext);
...
export async function opRead(path, rawOpts, ioIn) {
  ...
  if (!READABLE_EXTS.has(declared)) {
    throw new OfficeError(`office_read 不支持的扩展名 .${declared}（支持: docx/doc/odt/rtf/pdf/xlsx/xls/xlsb/ods/csv/tsv）`, 'UNSUPPORTED_FORMAT');
  }
  ...
  if (ext === 'pdf') return readPdfFile(io, path, readOpts);
 ...
 */
function wordBody({ path, markdown, opts, offset, info, meta, appendix, outline = outlineOf, label = 'Word 文档' }) {
  const total = markdown.length;
  if (opts.outline) {
    const items = outline(markdown);
    return {
      content: `已读取 ${label} ${path}${info}\n- 标题大纲: ${items.length} 条 / 正文共 ${total} 字符\n\n${outlineBlock(items)}`,
...
/** PDF 里没有 Markdown 标题,用页标记当大纲,便于按页定位与跳读。 */
export function outlineOfPages(text) {
  const items = [];
  let offset = 0;
  for (const line of text.split('\n')) {
    const hit = /^--- 第 (\d+) 页 ---$/.exec(line);
    if (hit) items.push({ level: 1, title: `第 ${hit[1]} 页`, offset });
    offset += line.length + 1;
  }
  // 只有一页时正文里没有页标记,整篇就是第 1 页
  return items.length || !text.trim() ? items : [{ level: 1, title: '第 1 页', offset: 0 }];
}

/**
 * PDF 读取:只抽文本层(页数 + 文本),后端顺序见 pdf.js。
 * offset/maxChars 与 Word 一致可分段读,outline 给的是页清单。
 */
async function readPdfFile(io, path, opts) {
  await ensureRegular(io, path, CAPS.MAX_EXCEL_INPUT_BYTES);
  const asHtml = opts.format === 'html';
  const offset = pageOffset(opts, asHtml);
  const buf = await io.readBuf(path);
  const r = await readPdfText(buf, { path });
  const notes = [
    `\n- 大小: ${fmtBytes(buf.length)}`,
    `\n- 解析方式: ${r.backend}, 页数: ${r.pages}`,
    containerNote(opts),
  ];
  if (r.messages.length) notes.push(`\n- 注: ${r.messages.join('；')}`);
  if (opts.withFormatting) notes.push('\n- 注: PDF 只抽文本层，没有字体 / 行距这类排版格式报告');
  const info = notes.join('');
  const meta = { kind: 'pdf', format: 'pdf', bytes: buf.length, pages: r.pages, backend: r.backend, messages: r.messages };
  const label = 'PDF 文档（仅文本层）';
  if (asHtml) return htmlWordBody(path, await md.plainTextToHtml(r.text), info, opts, meta, label);
  return wordBody({
    path,
    markdown: r.text,
    opts,
    offset,
    info,
    meta: { ...meta, words: roughWords(r.text) },
    outline: outlineOfPages,
    label,
  });
}
...
function htmlWordBody(path, html, info, opts, meta, label = 'Word 文档') {
