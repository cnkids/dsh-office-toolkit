 ...
 */
async function sniffFormat(io, path, ext) {
  ...
  const hit = ZIP_PART_EXTS.find(([part]) => partOf(zip, part));
  if (!hit) {
    return {
      ext,
      buffer,
      entries,
      error: `这个 .${ext} 是个 zip,但里面没有 Office 主文档(实际条目: ${entries.slice(0, 8).join('、') || '空'})。` +
        '多半是被改过后缀、或第三方工具生成的损坏文件 —— 请用 Word 打开后另存为 .docx 再试',
    };
  }
  const note = hit[1] === ext ? '' : `文件内容其实是 .${hit[1]}`;
  const repairNote = repaired.length ? `(自动修正了 ${repaired.length} 个非标准 zip 条目名)` : '';
  return { ext: hit[1], buffer, entries, note: `${note}${repairNote}` };
}
...
export async function opRead(path, rawOpts, ioIn) {
  ...
  if (sniffed.note) opts.containerNote = sniffed.note;
  ...
  return readLegacyTableFile(io, path, ext, opts);
}
