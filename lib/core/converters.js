...
async function jsRebuild(inputPath, outputPath, opts = {}) {
  ...
  await writeFile(outputPath, await word.writeDocx({ html }, { title: '转换文档', readImage: opts.readImage }));
 ...
 */
export async function wordConvert(inputPath, outputPath, opts = {}) {
  const errors = [];
  ...
  if (JS_WRITE_TARGETS.has(extOf(outputPath))) {
    try {
      return { backend: await jsRebuild(inputPath, outputPath, opts) };
    } catch (err) {
