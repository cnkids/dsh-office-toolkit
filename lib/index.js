  ...
  return {
    ...
    parameters: {
      ...
      properties: {
        ...
        maxChars: { type: 'integer', description: 'Word 正文每段返回的字符上限（分页大小），默认 90000' },
        offset: { type: 'integer', description: '仅 Word：从正文第几个字符开始返回，默认 0。返回里会给出总字符数与下次该传的 offset，用于分段读长文档；offset 超出总长会明确提示已到末尾' },
        outline: { type: 'boolean', description: '仅 Word 的 text 模式：只返回标题大纲（TSV：级别 / 字符偏移 / 标题），可据此传 offset 直接跳到某一章；与 format: "html" 不能同时用' },
