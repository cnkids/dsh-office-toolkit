// dsh-office-tools — DeepSeek Harness host plugin (host-only, no client half).
...
export function apply(ctx) {
  ...
  if (!tools || typeof tools.register !== 'function') {
    ctx.logger?.warn?.('[dsh-office-tools] tools 服务不可用，插件未注册');
    return;
  }
  const registerAll = () => {
    const disposers = [];
    ...
    ctx.logger?.info?.(`[dsh-office-tools] 已注册 ${disposers.length} 个 Office 工具`);
    return () => { for (const d of disposers) { try { typeof d === 'function' && d(); } catch { /* ignore */ } } };
  };
  if (typeof ctx.effect === 'function') {
    ctx.effect(registerAll, 'dsh-office-tools:tools');
  } else {
    registerAll();
  }
}
