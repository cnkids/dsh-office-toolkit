...
function readTool(ctx) {
  return {
    ...
    description:
      ...
      'Excel 以 TSV 代码块返回，可用 sheets/range/maxRows 分段读取大表。',
    parameters: {
      type: 'object',
      properties: {
...
function fillTemplateTool(ctx) {
  return {
    ...
    description:
      '用数据填充 .docx 模板中的 {{变量}} 占位符（docxtemplater）；模板套打用本工具。' +
      'data 的键即变量名，支持嵌套如 {{a.b}}。不支持循环/条件语法；批量套打时多次调用即可。',
    parameters: {
      type: 'object',
      properties: {
...
export function apply(ctx) {
  ...
  const registerAll = () => {
    ...
    if (missing.length) {
      ...
      ctx.logger?.warn?.(
        `[dsh-office-toolkit] 依赖安装不完整,缺少 ${detail};相关工具会报 MISSING_DEPENDENCY。` +
